...
L'algorithme _zazou_, décrite dans la section \@ref(zazousection), est implémenté dans le _package_ éponyme. Il bénéficie du package `{ggtree}` [@yu2017ggtree] pour mettre en regard l'arbre et différentes variables associées aux feuilles. Un exemple d'utilisation est donnée dans l'annexe \@ref(vignette).
...
Nous donnons ensuite nos $z$-scores à la fonction `estimate_shifts` pour déterminer la position idéale des sauts avec une régression lasso. La grille des $\ou{\alpha}$ est également spécifiée mais celles des $\lambda$ est déterminée automatiquement. 
...
La courbe ROC de _zazou_ est au dessus de celle de la méthode standard. Et ceci se traduit dans en terme d'AUC :
